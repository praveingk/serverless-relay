...
type Relay struct {
	...
	clientConn   net.Conn
	egress       MessageQueue // Message queued to be sent to gateway
	ingress      MessageQueue // Messages queued to be sent to client
...
func (r *Relay) StartRelay() error {
	...
	for {
		...
		clog.Info("Accept incoming connection", ac.LocalAddr().String(), "->", ac.RemoteAddr().String())
		if ac.RemoteAddr().Network() == r.gw {
...
func (r *Relay) Init(ip, port, gw, target string) {
	...
	r.clientConn = nil
	r.gatewayConn = nil
	r.egress.Init(queueSize)
	r.ingress.Init(queueSize)
}
